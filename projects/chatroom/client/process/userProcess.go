...
func (this *UserProcess) Register(userId int,
	...
	if err != nil {
		log.Println("注册发送信息错误 err=", err)
	}

	mes, err = tf.ReadPkg() // mes 就是 RegisterResMes

	if err != nil {
		log.Println("readPkg(conn) err=", err)
		return
	}

	//将mes的Data部分反序列化成 RegisterResMes
	var registerResMes message.RegisterRstMsg
	err = json.Unmarshal([]byte(mes.Data), &registerResMes)
	if registerResMes.Code == 200 {
		log.Println("注册成功, 你重新登录一把")
		os.Exit(0)
	} else {
...
func (this *UserProcess) Login(userId int, userPwd string) (err error) {
	...
	if n != 4 || err != nil {
		log.Println("conn.Write(bytes) fail", err)
		return
	}

	log.Printf("客户端，发送消息的长度=%d 内容=%s", len(data), string(data))

	// 发送消息本身
	_, err = conn.Write(data)
	if err != nil {
		log.Println("conn.Write(data) fail", err)
		return
	}
	...
	var loginResMes message.LoginRstMsg
...
func  Close() (err error) {
	//1 创建一个Mes
	var mes message.Message
	mes.Type = message.LoginExistMsgType

	mes.Data = strconv.Itoa(CurUser.UserId)

	//4. 对mes再次序列化
	data, err := json.Marshal(mes)
	if err != nil {
		log.Println("Close json.Marshal fail =", err.Error())
		return
	}

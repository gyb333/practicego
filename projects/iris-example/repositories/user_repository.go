...
// Query表示访问者和操作查询。
type QueryUser func(datamodels.User) bool
// UserRepository处理用户实体/模型的基本操作。
//它是一个可测试的接口，即内存用户存储库或 连接到sql数据库。
type UserRepository interface {
	Exec(query QueryUser, action QueryUser, limit int, mode int) (ok bool)
	Select(query QueryUser) (user datamodels.User, found bool)
	SelectMany(query QueryUser, limit int) (results []datamodels.User)
	InsertOrUpdate(user datamodels.User) (updatedUser datamodels.User, err error)
	Delete(query QueryUser, limit int) (deleted bool)
}
...
func (r *userMemoryRepository) Exec(query QueryUser, action QueryUser, actionLimit int, mode int) (ok bool) {
	loops := 0
...
//自从我第一次想到它以来，我一直在使用它，
//希望你会发现它也很有用。
func (r *userMemoryRepository) Select(query QueryUser) (user datamodels.User, found bool) {
...
//如果limit <= 0则返回所有内容。
func (r *userMemoryRepository) SelectMany(query QueryUser, limit int) (results []datamodels.User) {
...
func (r *userMemoryRepository) Delete(query QueryUser, limit int) bool {

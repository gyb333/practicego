...
import (
	"go-grpc-example/ILogger"
	"sync"
)

//Logger basic implementation of api.Logger interface
type Logger struct {
	instance ILogger.Logger // access only via Logger.logger()
	module   string
	once     sync.Once
}

// logger factory singleton - access only via loggerProvider()
var loggerProviderInstance ILogger.LoggerProvider
...
func loggerProvider() ILogger.LoggerProvider {
	loggerProviderOnce.Do(func() {
		...
		loggerProviderInstance = ILogger.LoggerProviderFunc()
...
func Initialize(l ILogger.LoggerProvider) {
	loggerProviderOnce.Do(func() {
...
func SetLevel(module string, level Level) {
	ILogger.SetLevel(module, ILogger.Level(level))
}
...
func GetLevel(module string) Level {
	return Level(ILogger.GetLevel(module))
}
...
func IsEnabledFor(module string, level Level) bool {
	return ILogger.IsEnabledFor(module, ILogger.Level(level))
}
...
func LogLevel(level string) (Level, error) {
	l, err := ILogger.ParseLevel(level)
	return Level(l), err
}
...
func (l *Logger) logger() ILogger.Logger {
	l.once.Do(func() {

...
func (ch *Channel) DeleteClient(client *Client) int {
	var ret int
	ch.ReplyMsg(fmt.Sprintf("从channel:%s 中删除client:%d ", ch.Name, client.Id))
...
func (ch *Channel) Notify(message string) bool {
	...
	for cid, _ := range ch.clients {
		ch.ReplyMsg(fmt.Sprintf("channel:%s client:%d message:%s", ch.Name, cid, message))
	}
	return true
